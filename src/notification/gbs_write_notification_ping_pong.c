...
main(int argc, char* argv[])
{
  ...
  for(i = 0; i < options.iterations + options.skip; ++i)
  {
    if(my_id == 0)
    {
      ...
      GASPI_CHECK(
          gaspi_write_notify(segment_id, 0, 0, segment_id, 0, 1, notification_id, notification_val, q_id, GASPI_BLOCK));
    ...
    else
    {
      GASPI_CHECK(gaspi_notify_waitsome(segment_id, notification_id, 1, &notification_id,
                                        GASPI_BLOCK));
      GASPI_CHECK(gaspi_write_notify(segment_id, 0, 0, segment_id, 0, 1, notification_id, notification_val, q_id, GASPI_BLOCK));
    }
