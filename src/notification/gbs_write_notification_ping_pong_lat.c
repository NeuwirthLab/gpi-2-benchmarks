...
main(int argc, char* argv[])
{
      ...
      size *= 2)
  {
    if(!options.single_buffer)
    {
      allocate_benchmark_memory(segment_id_dst, size * sizeof(char),
                                my_id == 0 ? 'a' : 'b');
      GASPI_CHECK(gaspi_segment_ptr(segment_id_dst, &ptr_dst));

      allocate_benchmark_memory(segment_id_src, size * sizeof(char),
                                my_id == 0 ? 'a' : 'b');
      GASPI_CHECK(gaspi_segment_ptr(segment_id_src, &ptr_src));
    }
    ...
    for(notification_id = 0; notification_id < options.iterations + options.skip; ++notification_id)
    {
      ...
      if(my_id == 0)
      {
        ...
        GASPI_CHECK(gaspi_write_notify(segment_id_src, 0, 1, segment_id_dst, 0, size, notification_id, notification_val, q_id, GASPI_BLOCK));
        
        GASPI_CHECK(gaspi_notify_waitsome(segment_id_dst, notification_id, 1, &first_notification_id,
                                          GASPI_BLOCK));
      ...
      else //id == 1
      {
        ...
        GASPI_CHECK(gaspi_write_notify(segment_id_src, 0, 0, segment_id_dst, 0, size, notification_id, notification_val, q_id, GASPI_BLOCK));
